...
run_turn(prompt) ───┤
                    └─ enter loop ──────────────────────────────────┐
                       │                                            │
                       │  step 1: manage_context (micro/soft/hard)   │
                       │  step 2: build CompletionRequest            │
...
pub(crate) struct TurnConfig {
    ...
    /// hard 水位的绝对值显式覆盖（token 数）。`None` 时按 compact_ratio 推算。
    pub compact_threshold_tokens: Option<u64>,
    /// hard 水位（同步压缩兜底）占 context_window 的比例。默认 `Some(0.85)`。详见 §4。
    pub compact_ratio: Option<f64>,
    /// 后台全量压缩开关 + soft 水位比例（默认开 / `Some(0.7)`）。详见 §4。
    pub background_compact_enabled: bool,
    pub compact_soft_ratio: Option<f64>,
    /// 微压缩开关 + 水位比例（默认开 / `Some(0.6)`）。详见 §4。
    ...
    /// before-turn-end hook 强制续命的硬上限。默认 3。详见 hook-step-context.md §5.7。
...
loop {
    ...
    manage_context().await?;   // micro → soft(后台) → hard(同步兜底)，详见 §4
...
## 4. 上下文管理（三档水位，编排在主循环，不在 History）

摘要要调 LLM，存储抽象（`History`）够不到 provider，所以编排放在主循环这层
（对齐 codex / opencode / Claude Code）。`History` 提供 `snapshot` / `replace` /
`splice_prefix` / `record_input_tokens` / `token_estimate`。主循环每轮开头调
`manage_context`，按三档水位分层处置（`session/turn/{compact,microcompact,compaction_slot}.rs`）：

```
estimate = token_estimate()
① estimate ≥ micro(0.6·W) 且 microcompact_enabled
     → run_microcompact()：清旧的大 tool_result，不调 LLM、不删消息（同步、最便宜）
       压完重取 estimate（可能已降到 soft/hard 之下）
② soft(0.7·W) ≤ estimate < hard(0.85·W) 且 background_compact_enabled
     → spawn_background_compaction()：异步起摘要压缩，**不阻塞本轮**（single-flight）
       本轮就此返回，摘要落地是稍后的事
③ estimate ≥ hard(0.85·W)
     → 已有后台压缩在飞 → await 它落地；否则同步 compact::run_sync 兜底
```

**三档阈值解析**（`compact_thresholds`）：`hard` 支持 `compact_threshold_tokens`
（绝对值）覆盖，否则 `context_window * compact_ratio`（默认 `0.85`）。`micro` / `soft`
只能从 `context_window * {microcompact,compact_soft}_ratio`（默认 `0.6` / `0.7`）推。
任一档拿不到（无 `context_window`、ratio 为 `None`、或对应开关关）即跳过该档。

**配置开关**（`[turn]`，详见 §9）：`microcompact_enabled` / `microcompact_ratio`、
`background_compact_enabled` / `compact_soft_ratio`、`compact_ratio`（hard）。后台压缩
与微压缩均可独立开关；全关时退化为仅 hard 水位的同步压缩（即旧行为）。
...
### 4.1 全量压缩（摘要，micro 之上的两档共用）

- `compact::plan`（纯函数）：`select_boundary` 把历史切成「待摘要 head」+「原样保留
  tail」。边界**对齐到轮次起点**（含非 ToolResult 内容的真实 user 消息），保证 tail
  以合法 user 轮开头、且绝不切散 `tool_use`↔`tool_result` 配对（两个 wire codec 都
  不校验配对，必须自保）。tail 预算 `clamp(threshold/4, 2k, 8k)`。无安全边界 → `None`。
- `compact::summarize`：用当前 provider/model 对 head 跑一次 `tool_choice=None` 的子
  请求，按固定结构化 markdown 模板产摘要；检出旧摘要（`SUMMARY_PREFIX` 起头）走
  `<previous-summary>` 增量合并；head 里的超长 tool_result 截到 ~2k 字符、图片剥离。
- 回写：`history.splice_prefix(plan.drop_count, summary_msg)`——用合成 assistant 摘要
  消息（`SUMMARY_PREFIX + 正文`）换掉**当前**列表前 `drop_count` 条、保留其后全部。

**后台压缩并发不变式**：`drop_count` 在某时刻的 snapshot 上算得；摘要 LLM 调用的几秒
里前台 turn 仍在尾插（append）、或做微压缩（原地内容替换）。`splice_prefix` 只丢前
`drop_count` 条、保留尾部，故这期间新增的尾部消息不丢。其合法前提是「飞行期间只发生
尾插与原地替换，不增删中段」——唯一会删中段的操作是压缩本身，而后台压缩 **single-
flight**（`CompactionSlot` 同时至多一个在飞）。这条不变式是后台压缩的地基。

### 4.2 微压缩（microcompact，最便宜的第一道防线）

不调 LLM、不删消息：扫历史，跳过最近 `KEEP_RECENT_TURNS`（=3）个轮次，把更老轮次里
体积 > `MIN_CLEAR_TOKENS`（=512）的 `tool_result` 正文换成占位符（保 `tool_use_id` /
`is_error`）。占位文本即幂等 sentinel。纯 `O(n)`，发 `ContextMicrocompacted` 事件。

**约定**：

- 失败（无边界 / provider 错 / 空摘要 / 取消 / 无可清理项）一律最佳努力降级、跳过本次，
  不杀 turn
- 压缩只在 LLM 调用之前发生，不打断已经 in-flight 的工具
- 子 agent turn（`spawn_agent`）不做后台压缩（上下文短、生命周期随工具调用结束），仅享
  hard 水位的同步压缩兜底
...
| `compact_threshold_tokens` | `None` | hard 水位绝对值显式覆盖；`None` 时按 `compact_ratio` 推算 |
| `compact_ratio` | `Some(0.85)` | hard 水位 = `context_window * 0.85`；模型不公开 window 时不压缩 |
| `background_compact_enabled` | `true` | 越 soft 水位异步起摘要压缩，不阻塞当轮 |
| `compact_soft_ratio` | `Some(0.7)` | soft 水位 = `context_window * 0.7`（须 < hard 才有意义） |
| `microcompact_enabled` | `true` | 越 micro 水位清旧的大 tool_result，不调 LLM |
| `microcompact_ratio` | `Some(0.6)` | micro 水位 = `context_window * 0.6` |
...
| `max_hook_continues` | `3` | before-turn-end hook 强制续命硬上限；防 hook 无限 `Continue` 把 turn 拖成死循环 |
