...
| Tool（MCP） | stdio / http / sse 三种 transport，按 `enabled_servers` 白名单装配 | `[mcp]` | `defect_cli::mcp_servers` |
...
- 想缩二进制体积？我们后续会给 `defect-cli` 加可选 feature，按 provider / tool 维度编译期裁剪——届时跟着 feature flag 关即可，不需要自己在 `Cargo.toml` 里改
